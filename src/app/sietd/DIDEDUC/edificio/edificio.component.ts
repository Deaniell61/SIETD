...
export class EdificioComponent implements OnInit {
  ...
  constructor(
    ...
    private mainService: EdificioService,
    ...
    ngOnInit() {
      ...
      this.DanioService.getAll()
                  .then(response => {
                  this.DanioCombo = response
                  }).catch(error => {
                  console.clear     
                  this.createError(error) 
      })
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/edificio", "_blank");
  }
    ...
    subirImagenes(archivo,id){
      ...
      let url = `${this.basePath}/api/carga/edificio`
      let bar:any
      var i=0;
