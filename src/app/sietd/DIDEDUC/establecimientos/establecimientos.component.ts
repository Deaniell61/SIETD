...
export class EstablecimientosComponent implements OnInit {
  ...
  constructor(
    ...
    private mainService: EstablecimientoService,
    private grandParentService: DepartamentosService,
    private muniService: MunicipiosService,
    private EstaService: TipoEstablecimientoService,
    private EducaService: NivelEducativoService,
    private EdifiService: EdificioService
  ) { }
    ...
    ngOnInit() {
      ...
      this.EstaService.getAll()
                  .then(response => {
                  this.EstablecimientoCombo = response
                  }).catch(error => {
                  console.clear     
                  this.createError(error) 
      })

      this.EducaService.getAll()
                    .then(response => {
                    this.EducativoCombo = response
                    }).catch(error => {
                    console.clear     
                    this.createError(error) 
      })

      this.EdifiService.getAll()
                  .then(response => {
                  this.EdificioCombo = response
                  }).catch(error => {
                  console.clear     
                  this.createError(error) 
      })
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/establecimiento", "_blank");
  }
    subirImagenes(archivo,id){
      $('#Loading').css('display','block')
      $('#Loading').addClass('in')
      var archivos=archivo.srcElement.files;
      let url = `${this.basePath}/api/carga/establecimiento`
      let bar:any
      var i=0;
