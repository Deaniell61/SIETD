...
import { DetallemidesService, LocalUserObject } from './../_services/detallemides.service';
...
@Component({
  selector: 'app-detallemides',
  templateUrl: './detallemides.component.html',
  styleUrls: ['./detallemides.component.css']
})
export class DetallemidesComponent implements OnInit {
 ...
 console.log(this.LocalUserObject)
      ...
      this.Programamide.getAll()
                              .then(response => {
                                this.ProgramaCombo = response
                              }).catch(error => {
                                console.clear     
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/detalle_mides", "_blank");
  }
    ...
    subirImagenes(archivo,id){
      ...
      let url = `${this.basePath}/api/carga/detalle_mides`
      let bar:any
      var i=0;
