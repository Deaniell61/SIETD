...
import { HogarescomunitariosService, LocalUserObject } from './../_services/hogarescomunitarios.service';
...
@Component({
  selector: 'app-hogarescomunitarios',
  templateUrl: './hogarescomunitarios.component.html',
  styleUrls: ['./hogarescomunitarios.component.css'],
})
export class HogarescomunitariosComponent implements OnInit {

  LocalUserObject: LocalUserObject;

  title = "Hogares Comunitarios"
  Table:any
  selectedData:any
  ...
  constructor(
    ...
    private mainService: HogarescomunitariosService,
    ...
    subirImagenes(archivo,id){
      ...
      let url = `${this.basePath}/api/carga/hogares_comunitarios`
      let bar:any
      var i=0;
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/hogares_comunitarios", "_blank");
  }
    cargarAll(){
      $('#Loading').css('display','block')
