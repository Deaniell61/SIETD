...
@Component({
  ...
  templateUrl: './acueducto.component.html',
  styleUrls: ['./acueducto.component.css']
})
export class AcueductoComponent implements OnInit {
  title = "Acueducto"
  Table:any
  selectedData:any
  ...
  constructor(
    ...
    private mainService: AcueductoService,
  ...
  ngOnInit() {
    ...
    this.muniService.getJustAll()
     ...
     this.AreService.getAll()
                         .then(response => {
                         this.AreaCombo = response
                         }).catch(error => {
                         console.clear     
                         this.createError(error) 
})
  ...
  onNavigate(){
    window.open("http://backend.sietd.alexmejicanos.com/api/download/acueducto", "_blank");
}  
...
subirImagenes(archivo,id){
  ...
  let url = `${this.basePath}/api/carga/animal`
  let bar:any
  var i=0;
