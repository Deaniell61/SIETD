...
import { DetalleCombustibleService, LocalUserObject } from './../_services/detalle-combustible.service';
...
@Component({
  selector: 'app-detalle-combustible',
  templateUrl: './detalle-combustible.component.html',
  styleUrls: ['./detalle-combustible.component.css']
})
export class DetalleCombustibleComponent implements OnInit {

  LocalUserObject: LocalUserObject;

  title = "Detalle Combustible"
  Table:any
  selectedData:any
  ...
  constructor(
    ...
    private mainService: DetalleCombustibleService,
    private parentService: CombustibleService,
    ...
    console.log(this.LocalUserObject)
      ...
      .then(response => {
        this.CombustibleCombo = response
      }).catch(error => {
        console.clear     
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/detalle_combustible", "_blank");
  }
    ...
    subirImagenes(archivo,id){
      ...
      let url = `${this.basePath}/api/carga/detalle_combustible`
      let bar:any
      var i=0;
