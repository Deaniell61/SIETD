...
import { DetalleInfraestructuraService, LocalUserObject } from './../_services/detalle-infraestructura.service';
...
@Component({
  selector: 'app-detalles-infraestructuras',
  templateUrl: './detalles-infraestructuras.component.html',
  styleUrls: ['./detalles-infraestructuras.component.css']
})
export class DetallesInfraestructurasComponent implements OnInit {

  LocalUserObject: LocalUserObject;

  title = "Detalle Infraestructura"
  Table:any
  selectedData:any
  ...
  constructor(
    ...
    private mainService: DetalleInfraestructuraService,
    private muniService: MunicipiosService,
    private grandParentService: DepartamentosService,
    private FenoService: FenomenoService,
    private NiveService: NivelProductivoFamiliaService,
    private InfraService: InfraestructuraService
    ...
    console.log(this.LocalUserObject)
      ...
      this.FenoService.getAll()
      .then(response => {
        this.FenomenoCombo = response
      }).catch(error => {
        console.clear     
        this.createError(error) 
      })

      this.NiveService.getAll()
      .then(response => {
        this.NivelCombo = response
      }).catch(error => {
        console.clear     
        this.createError(error) 
      })

      this.InfraService.getAll()
      .then(response => {
        this.InfraestructuraCombo = response
      }).catch(error => {
        console.clear     
        this.createError(error) 
      })
    ...
    onNavigate(){
      window.open("http://backend.sietd.alexmejicanos.com/api/download/detalle_infraestructura", "_blank");
  }
    cargarAll(){
    ...
    subirImagenes(archivo,id){
      ...
      let url = `${this.basePath}/api/carga/detalle_infraestructura`
      let bar:any
      var i=0;
